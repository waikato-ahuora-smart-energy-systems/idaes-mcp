...
# Calculate the saturation pressure of water at 0 and 75 degrees C
saturation_pressure_0 = CP.PropsSI('P', 'T', 0 + 273.15, 'Q', 0, fluidR)
print(f"The saturation pressure of water at {0} degrees Celsius is: {saturation_pressure_0} Pa")
saturation_pressure_75 = CP.PropsSI('P', 'T', 75 + 273.15, 'Q', 0, fluidR)
print(f"The saturation pressure of water at {75} degrees Celsius is: {saturation_pressure_75} Pa")

#Specify constraints
m.fs.compressor.outlet.pressure.fix(saturation_pressure_75 * units.Pa) # compressor makes it go to a higher pressure, which will heat it up
m.fs.pressureChanger.outlet.pressure.fix(saturation_pressure_0 * units.Pa) # pressure changer makes it go to a lower pressure
...
m.fs.cooler.outlet.enth_mass.fix(m.fs.helmholtz_water.htpx(p=saturation_pressure_75 * units.Pa,x=0,amount_basis=AmountBasis.MASS,with_units=True))
m.fs.heater.outlet.enth_mass.fix(m.fs.helmholtz_water.htpx(p=saturation_pressure_0 * units.Pa,x=1,amount_basis=AmountBasis.MASS,with_units=True))
...
tear_guesses = {
    "flow_mass": {0:1},
    "enth_mass": {0: value(m.fs.helmholtz_water.htpx(p=saturation_pressure_0 * units.Pa,x=1,amount_basis=AmountBasis.MASS,with_units=True))},
    "pressure": {0: saturation_pressure_0 },
}
